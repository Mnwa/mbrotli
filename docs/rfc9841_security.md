...
Shared dictionaries are not implemented. When they are, the following will
apply and this section will be rewritten as behaviour rather than intent:
...
- **CRIME-style side channels.** Compressing attacker-controlled data together
  ...
  dictionary does not make the payloads safe to mix.
- **Resource exhaustion.** Serialized dictionaries carry lengths and counts
  read from bytes. Every one of them has to be checked against a configured
  limit before anything is allocated for it.
