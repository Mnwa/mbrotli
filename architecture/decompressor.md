...
`Bits` uses a demand-driven `u128` reservoir and accepts only bytes required for
one field. This supports 62-bit extra fields without reading beyond a member.
Small headers are parsed transactionally; partial peeks retain bits, and a
validated field commits by dropping exactly its width. Padding must be zero.
Huffman trees use canonical length counts and sorted symbols with inline bounded
storage. A one-symbol tree consumes zero symbol bits. Builder phases retain
unfinished symbol lists/repeats; reset abandons every partial description.
Block-type and context-map parsers have separate named resumable phases.

The stream has explicit command/literal/distance/copy phases and counters.
Regenerated bytes update caller output, bounded history, and the last two literal
context bytes together. Raw blocks update the same history as compressed blocks.
History grows on actual output and retains every still-addressable byte; declaring
a large window does not allocate it eagerly. Distances and cumulative positions
use `u64`; arithmetic uses checked operations and `u128` for wire distances.
The theoretical large alphabet is `16 + NDIRECT + (124 << NPOSTFIX)` even for
small large-window headers. Every reachable distance symbol must have a maximum
...
count current owned capacities. History, context maps, Huffman groups, and rare
prefix-crossing scratch reuse capacity. There is no allocation per output byte or
transform. Header-only and metadata-only streams need no heap workspace.
...
The decoder currently uses scalar regeneration and canonical per-bit Huffman
lookup for every selected backend; no decoder SIMD acceleration is implemented.
Compatibility evidence and remaining acceptance gaps are tracked separately in
